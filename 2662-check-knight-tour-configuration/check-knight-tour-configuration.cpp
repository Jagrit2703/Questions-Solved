...
public:
    bool solve(int i, int j, int k, int &m, int &n, vector<vector<int>>&g){
        //valid moves = 2 horizontal 1 vertical
        //valid moves = 2 vertical 1 horizontal 
        // create visited and mark them, create moves as 
        //solve(i+2,j+1), solve(i+1,j+2), solve(i+2,j-1), solve(i+1,j-2), solve(i-2,j-1)
